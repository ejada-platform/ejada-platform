    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            for (const record of attendanceResponse.data) {
                const circleId = record.circle;
                if (!newAttendanceMap.has(circleId)) {
                    newAttendanceMap.set(circleId, record.date);
                }
            }
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            ...
            <p className="text-gray-600 mb-6">
                This page shows the last time attendance was recorded for each circle, indicating teacher activity.
            </p>
            <div className="bg-white p-6 rounded-lg shadow">
                <table className="w-full text-left">
                    ...
                    <tbody>
                        {circles.map(circle => {
                            // This is the corrected variable that was causing the error
                                    ...
                                    <td>
                                        {lastAttendanceDate ? (
                                            <span className="font-mono bg-green-100 text-green-800 px-2 py-1 rounded">
                                                {new Date(lastAttendanceDate).toLocaleDateString()}
                                            </span>
                                        ) : (
